...
import model
from model.help_funcs import Transformer, TransformerDecoder, TwoLayerConv2d
...
if __name__ == '__main__':
    ...
    net = BASE_Transformer(input_nc=3, output_nc=2, token_len=4, resnet_stages_num=4, with_pos='learned') #ResNet(input_nc=3, output_nc=2, output_sigmoid=False)
    rgb = torch.randn((1, 3, 128, 128))
    out = net(rgb,rgb)
    print(out.shape) #torch.Size([1, 2, 128, 128])
    sflops, sparams = profile(net, inputs=(rgb,rgb))
    print("参数量：{} M 计算量： {} G".format(sparams / 1e+6, sflops / 1e+9))